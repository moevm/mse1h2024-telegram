...
export default class TableCreator {
  ...
  changeTable(table: TableItem): void {
    const existingTable: TableItem | undefined = this.data.find(
  ...
  addTableRule(tableId: string, page: Page): void {
    const existingTable: TableItem | undefined = this.data.find(
      (existingTable) => existingTable.id === tableId
    )
    if (existingTable) {
  ...
  changeTableRule(tableId: string, page: Page): void {
    ...
    if (existingTable) {
      const existingPage: Page | undefined = existingTable.pages.find(
